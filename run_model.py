...
def evaluate(model, data_loader, loss_fct, device):
    ...
    for batch in tqdm(data_loader, desc='Validation', leave=False):
        ...
        masked_classes = batch.pop('masked_classes').to(device)
...
def train(model, train_loader, loss_fct, optimizer, device):
    ...
    for batch in tqdm(train_loader, desc='Training', leave=False):
        ...
        # mlm_loss = loss_fct(prediction_scores.view(-1, model.config.vocab_size), true_input_ids.view(-1))
        object_logits = model(batch)
        print("Object logits shape:", object_logits.shape, "dtype:", object_logits.dtype)
        print("Masked classes shape:", masked_classes.shape, "dtype:", masked_classes.dtype)
        
        object_class_loss = loss_fct(object_logits, masked_classes)
        # total_loss = mlm_loss + object_class_loss
        total_loss = object_class_loss
...
def main():
    ...
    train_dataset = VisualStoryDataset(f"{args.input_dir}/train-sample.json", args.input_dir, tokenizer)
    val_dataset = VisualStoryDataset(f"{args.input_dir}/val-sample.json", args.input_dir, tokenizer)
    ...
    print(f"Number of object classes: {num_object_classes}")
    sys.exit()
